...
export default function UpdatesView({ updates, okrs, profiles, mgmtReports, weekLabel }: Props) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex border border-surface-2 rounded-sm overflow-hidden">
          <button
            ...
            className={`px-4 py-1.5 text-xs font-medium transition-colors ${viewBy === 'dri' ? 'bg-ink text-paper' : 'bg-white text-muted hover:text-ink'}`}
          >
            By DRI
          </button>
          <button
            onClick={() => setViewBy('entity')}
            className={`px-4 py-1.5 text-xs font-medium transition-colors ${viewBy === 'entity' ? 'bg-ink text-paper' : 'bg-white text-muted hover:text-ink'}`}
          >
            By Entity
          </button>
        </div>
      ...
      {viewBy === 'dri' && allUserIds.length > 0 && (
      ...
      {viewBy === 'entity' && entities.length > 0 && (
